...
public class TestRoot {
	...
	protected static boolean setup(String propertiesFile){
		...
		if(alertTitle != null && alertTitle.getText().contains("Connection problem.")){
			System.out.println("Pausing to see if connectivity problem will resolve itself...");
			sleep(5000);
			behavior.Page.popupOk(driver);
		}
		
		
		// Set the location to the iHeart Office in NYC
		/*if(useEmulator || waitForVisible(driver, MobileBy.name("Set Location"), 5) != null){
	...
	/*private static void enterZip(){
		waitForVisible(driver, MobileBy.id(Page.controllerId + "zipcode_promt_edit_text"), 10).sendKeys("10013");
		click(driver, MobileBy.name("Set Location"));
	}*/
	...
	public static boolean isMarshmallow(){
		String platform = TestRoot.driver.getCapabilities().getCapability("platformVersion").toString();
		int version = 0;
		try{
			version = Integer.parseInt(platform.substring(0, 1));
		}
		catch(Exception e){}
	...
	public static void waitToClick(AndroidDriver<MobileElement> d, By by, int duration){
		// Make sure there isn't a popup window TODO
